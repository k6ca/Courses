/*Uma aplicação necessita de maior segurança através uma autenticação, para confirmação de cadastro de usuário. 
Você como programador deve desenvolver um sistema de CAPTCHA (Completely Automated Public Turing test to tell Computers and Humans Apart), 
que consiste em autenticação por desafio e resposta, o qual o usuário deve utilizar.

O CAPTCHA deve gerar uma sequência aleatória de 4 figuras geométricas (não repetidas), dentre 8 figuras geométricas possíveis, que deve ser armazenada. 
Para autenticação o usuário deve repetir o CAPTCHA gerado, digitando a sequência de 4 figuras na ordem correta, uma por vez.

Receber os dados de cadastro do usuário (nome de usuário e senha) e em seguida gerar o CAPTCHA. Solicitar ao usuário que repita o CAPTCHA gerado. 
Caso a sequência esteja correta, mostrar mensagem de cadastro realizado. Caso a sequência esteja errada, mostrar a sequência gerada e a sequência 
digitada "A sequência gerada era X, você digitou Y" (exemplo) e perguntar ao usuário se deseja gerar um novo CAPTCHA ou voltar ao menu principal.
